...
class SpaceDelimitedParser(AbstractParser):
    ...
    Supports languages such as English, French, Spanish, etc.
    """
    ...
    @staticmethod
    @functools.lru_cache
    def compile_re_pattern(pattern: str, *args, **kwargs) -> re.Pattern[str]:
    ...
    def get_default_word_characters() -> str:
        ...
        def add_current_to_ranges() -> None:
            if current is None:
                return
            ...
            if current[0] == current[1]:
                ...
                end_code = ucode(current[1])
                range_string = f"{start_code}-{end_code}"
    ...
    def preg_match_capture(self, pattern: str, subject: str) -> list[tuple[str, int]]:
        """Return the matched text and their start positions in the subject.

        E.g. search for r'cat' in "there is a CAT and a Cat" returns:
        [('CAT', 11), ('Cat', 21)]
        """
        compiled = self.compile_re_pattern(pattern, flags=re.IGNORECASE)
        return [(match.group(), match.start()) for match in compiled.finditer(subject)]

    def _parse_to_tokens(self, text: str, lang: Language) -> list[ParsedToken]:
        """Returns ParsedToken array for given language."""
        for replacement in lang.character_substitutions.split("|"):
    ...
    def parse_para(self, text: str, lang: Language) -> list[ParsedToken]:
        """Parse a string, appending the tokens to the list of tokens."""
        termchar = (lang.word_characters.strip() or
                   self.get_default_word_characters())

        splitex = lang.exceptions_split_sentences.replace(".", "\\.")
        pattern = (rf"({splitex}|[{termchar}]*)" if splitex.strip()
                  else rf"([{termchar}]*)")

        matches = self.preg_match_capture(pattern, text)
        wordtoks = [t for t in matches if t[0]]
...
class TurkishParser(SpaceDelimitedParser):
    """Parser to handle Turkish parsing specifics."""
